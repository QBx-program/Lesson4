...
def all_currency():
    ...
    for i, v in enumerate(a_cur.items()):
        print(f'{v[0]} = {a_cur[v[0]][2]} рублей за {a_cur[v[0]][0]} {a_cur[v[0]][1]} на {a_cur[v[0]][3]}')

def answer_rate(g_cur):
    a_cur = cur_rate.currency_rates()
    if a_cur.get(g_cur):
        print(f'{g_cur}/RUB = {a_cur[g_cur][2]} рублей за {a_cur[g_cur][0]} {a_cur[g_cur][1]} на {a_cur[g_cur][3]}')
    else:
